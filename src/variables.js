module.exports = {
	initVariables() {
		...
		outputChannels.forEach((channel) => {
			...
			variables.push({
				variableId: `output_audio_${channel}_current_input`,
				name: `Output Channel ${channel} - Current Audio Input`,
			})
			variables.push({
				variableId: `output_video_${channel}_current_input`,
				name: `Output Channel ${channel} - Current Video Input`,
			})
		})
