module.exports = {
	initVariables() {
		...
		outputChannels.forEach((channel, i) => {
			if (channel === 0) return;
			variables.push({ variableId: `output_${channel}_video_input`, name: `Output Channel ${channel} - Video Input` });
			variables.push({ variableId: `output_${channel}_audio_input`, name: `Output Channel ${channel} - Audio Input` });
